...
**세마포어**
- 세마포어 변수, semWait연산, semSignal연산이 있다.
- 세마포어 변수 : 공유 가능한 자원의 수
- semWait연산 : 세마포어 값을 감소 시킨다. 음수가되면 호출한 프로세스는 블록된다.
- semSignal연산 : 실행되던 프로세스가 종료되어, 세마포어 값을 증가시킨다. 만약 값이 0이거나 음수면, swmWait연산에 의해 블록된 프로세스를 wake_up한다
